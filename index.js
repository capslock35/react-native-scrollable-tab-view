...
class ScrollableTabView extends Component {
  ...
  _composeScenes() {
    return this._children().map((child, idx) => {
      ...
      if (!!this.props.collapsableBar) {
        element = this.state.tabsVisited.includes(idx)  ? child : null
      } else {
  ...
  _onChangeTab(prevPage, currentPage) {
    let tabsVisited=this.state.tabsVisited;
    if(!tabsVisited.includes(currentPage)){
      tabsVisited.push(currentPage);
    }
    this.setState({tabsVisited});
